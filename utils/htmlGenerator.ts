...
export const generateEbayHTML = (data: ListingData): string => {
  ...
  const comparisonHeaderHtml = data.comparisonItems.map(item => `
    <td style="padding:20px 10px; vertical-align: top; width: 18%;">
        <a href="${item.link}" target="_blank" style="text-decoration:none;">
            <img src="${item.image}" alt="${item.imageAlt || item.title}" style="width:100%; max-width:160px; height:160px; object-fit:contain; border-radius:6px; border: 1px solid #eee;">
            <p style="margin:0 auto; padding: 10px 0; font-size:13px; color:#000000; text-align: center; line-height: 1.2; height: 45px; overflow: hidden;">${item.title}</p>
            <button style="background:#008bfd; border:none; padding:8px 10px; color:white; border-radius:6px; cursor:pointer; font-weight:bold; font-size:11px; width: 100%;">Click here</button>
        </a>
    </td>
  `).join('');
...
<div style="font-family: 'Verdana', sans-serif; color: #333; max-width: 1000px; margin: 0 auto; padding: 0; margin-bottom: 0;">
    <h2 style="padding: 0; margin: 0 auto; text-align: center; width: 100%;">
    ...
    <div style="width:100%; margin:0 auto; padding:0; text-align:center; overflow-x:auto;">
        <table style="border-collapse:collapse; font-size:14px; text-align:center; width: 100%; min-width: 900px; margin:0; padding:0;">
            <tbody>
                <tr>
                    <td style="width: 10%; min-width: 100px;"></td> <!-- Empty corner -->
